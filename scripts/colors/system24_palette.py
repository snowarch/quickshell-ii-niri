...
OUTPUT_FILE = Path(
    os.environ.get(
        ...
        "~/.config/vesktop/themes/ii-palette.css",
    )
).expanduser()

# Legacy path for backwards compatibility
LEGACY_OUTPUT = Path("~/.config/vesktop/themes/system24-palette.css").expanduser()
...
def _build_palette(colors: Dict[str, str]) -> Dict[str, str]:
    ...
    # Discord brand color overrides (for buttons, links, etc)
    palette["--brand-360"] = _adjust_lightness(primary, 0.04)
    palette["--brand-400"] = primary
    palette["--brand-500"] = _adjust_lightness(primary, -0.06)
    palette["--brand-560"] = _adjust_lightness(primary, -0.12)

    # Additional Discord variables
    palette["--background-accent"] = primary
    palette["--background-surface-high"] = surface_high
    palette["--background-surface-low"] = surface_low
    palette["--background-base-lower"] = surface
    palette["--background-base-primary"] = surface_low
...
def _write_palette(palette: Dict[str, str]) -> None:
    ...
    lines = [
        "/*",
        " * ii-niri Discord Palette",
        " * Auto-generated by system24_palette.py — do not edit manually",
        " * Colors sync with your wallpaper via Material You",
        " */",
        ":root {",
    ]
    ...
    if LEGACY_OUTPUT != OUTPUT_FILE:
        _ensure_parent(LEGACY_OUTPUT)
        with LEGACY_OUTPUT.open("w", encoding="utf-8") as fh:
            fh.write(content)
